class DataInvalidaException(Exception):
    def __init__(self, msg):
        # conceito de herança -> DataInvalidaException herda tudo que a classe Exception tem
        super().__init__(msg)


class Data:
    # Atributo de classe -> self.__class__.__propriedade/atributo (melhor opção) ou NomeDaClasse.__propriedade/atributo
    __limite = {1: 31, 2: 28, 3: 31, 4: 30, 5: 31, 6: 30,
                7: 31, 8: 31, 9: 30, 10: 31, 11: 30, 12: 31}

    def __init__(self, dia: int, mes: int, ano: int):
        self.__mes = mes
        # método para validar o mês que foi passado durante a instaciação do objeto
        self.__validaMes(mes)
        # Vai usar o método setter setDia para atribuir um valor ao atributo dia e validá-lo
        self.setDia(dia)
        self.__ano = ano
    ...
    def setDia(self, dia: int):
        if dia > self.__class__.__limite[self.__mes]:
            raise DataInvalidaException(
                f"O mês {self.__mes} só tem {self.__class__.__limite[self.__mes]} dias.")
        self.__dia = dia

    def __validaMes(self, mes: int):
        if mes < 1 or mes > 12:
            raise DataInvalidaException(
                f"Mês {mes} é inválido. Valor correto: 1 <= mes <= 12")
